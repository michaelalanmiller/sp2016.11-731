...
def new_hyp(h, phrase, idx, size, esp, fromIndex, toIndex, span_future_logprob):
    '''
    input:
    ...
    return logprob, lm_state, hypothesis(logprob, heuristic_logprob, new_bit_array, future_logprob, lm_state, h, phrase, esp)
...
def retrans(winner):
    ...
    for i, (en, es) in enumerate(phrases):
        ...
        nextphrase = ()
        if i > 0: 
            prevphrase = tuple(phrases[i-1][0].english.split(' '))
        if i < len(phrases)-1: 
            nextphrase = tuple(phrases[i+1][0].split(' '))

        poss = [p for p in tm[es] if p.logprob != 0.0]
        if not poss:
            poss = tm[es]
        poss_scores = []
        ...
        for p in poss:
            score = p.logprob + \
                    .1*lm.score_phrase(prevphrase + tuple(p.english.split(' ')) + nextphrase)[1]
                    #lm.score_phrase_dir(prevphrase + tuple(p.english.split(' ')) + nextphrase)[1]
                    # lm.score_phrase_dir(tuple(p.english.split(' ')))[1]
            #if i == 0:
            #    score = p.logprob + lm.score(tuple(p.english.split(' ')), nextphrase[0])[1]
            #    score = p.logprob + lm.score_phrase(tuple(p.english.split(' ')), nextphrase[0])[1]
            #elif i == len(phrases)-1:
            #    score = p.logprob + lm.score((prevphrase[0].english,), p.english)[1]
            #else:
            #    score = p.logprob + lm.score(tuple(prevphrase[0].english.split(' ')) + tuple(p.english.split(' ')), nextphrase[0])[1]

            poss_scores.append(score)

        bestposs = poss[poss_scores.index(max(poss_scores))]
        # Update phrases with new best choice
...
hypothesis = namedtuple('hypothesis', 'logprob, heuristic_logprob, coverage_vector, future_logprob, lm_state, predecessor, phrase, src')
    ...
    for i, stack in enumerate(stacks[:-1]):
                        ...
                        for j in xrange(k + 1, N + 1):
                            ...
                            if f[k:j] in tm: # Find second phrase for reordering
                                ...
                                for phrase in tm[f[k:j]]:
                                    _, _, new_hypothesis = new_hyp(h, phrase, j, N, f[k:j], k, j, span_future_logprob)
                                ...
                                for hi in reordering_hyps:  # TODO: Modify this
                                    ...
                                    for phrase in tm[f[i:k]]:
                                        logprob, lm_state, new_hypothesis = new_hyp(hi, phrase, k, N, f[i:k], i, k, span_future_logprob)
        ...
        for h in heapq.nlargest(opts.s, stacks[i].itervalues(), key=lambda h: h.logprob + h.heuristic_logprob):
            for k in xrange(i + 1, N + 1):
                if f[i:k] in tm:
                    for phrase in tm[f[i:k]]:
                        logprob, lm_state, new_hypothesis = new_hyp(h, phrase, k, N, f[i:k], i, k, span_future_logprob)
